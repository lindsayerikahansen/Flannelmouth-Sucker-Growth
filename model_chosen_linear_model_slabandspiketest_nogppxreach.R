...
model{
  ...
  beta_turb ~ dnorm(0,0.01)
  beta_temp2 ~ dnorm(0,0.01)
  beta_gpp ~ dnorm(0,0.01)
  beta_gpp2 ~ dnorm(0,0.01)
  beta_turb2 ~ dnorm(0,0.01)
  ...
  for(i in 1:nreach) {
    for(j in 1:nmonth){
      ...
      #r2
      pred_int [i,j] <- beta0[i] + beta_temp* temp[i,j] + beta_turb* turb[i,j] + beta_gpp* GPP[i,j] + eps[i,j]
      pred_slope[i,j] <- beta_Li[i] + beta_temp2* temp[i,j] + beta_turb2* turb[i,j] + beta_gpp2 * GPP[i,j] + eps2[i,j]
    }
  }
